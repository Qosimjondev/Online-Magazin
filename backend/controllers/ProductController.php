...
class ProductController extends Controller
{
            ...
            if ($model->load($this->request->post())) {
                ...
                if(!empty($images)){
                    ...
                    foreach ($images as $image){
                        ...
                        $image->saveAs(Yii::getAlias('@frontend').'/web/img/product/'.$image->baseName."{$i}.".$image->extension, true);
                        $i++;
                    }
                    $model->image = implode(',', $mas);
                    $model->save();
    ...
    public function actionDelete($id)
    {
        ...
        for ($i = 0; $i < count($images); $i++){
            unlink(Yii::getAlias('@frontend').'/web/img/product/'.$images[$i]);
        }
        $model->delete();
